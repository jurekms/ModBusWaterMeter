...
typedef enum {
	...
	MB_ERR_LEGAL_FUNCTION				=  0x71,			// know modbus function
...
typedef enum {
	MB_FN_READ_COILS 						= 0x01,			// requesting the ON/OFF status of discrete coil - output
	MB_FN_READ_DISCRET_INPUTS 				= 0x02,			// requesting the ON/OFF status of discrete inputs -input (multiple binary inputs)
	MB_FN_READ_HOLDING_REGISTERS			= 0x03,			// requesting the content of analog input holding registers (multiple cells memory)
	...
	MB_FN_READLAVE_STATUS					= 0x07,			// requesting status of SLAVE device
	MB_FN_DIAGNOSTIC_TEST					= 0x08,			// requesting diagnostic test
	...
	MB_FN_IDENTIFY_SLAVE					= 0x11,			// requesting SLAVE identify
} TmbFunction;
...
//definition of pointer to callback function
...
//global variables
extern const TMBPtrCallbackFunction MB_CALLBACK_FUNCTIONS[18];
